...
CREATE OR REPLACE STREAMING LIVE TABLE customer_patterns_silver_copy
(
...
FROM stream(banking.customer_patterns_bronze)
;

-- COMMAND ----------

CREATE OR REFRESH STREAMING TABLE customer_patterns_silver
TBLPROPERTIES ("quality" = "silver")
COMMENT "Clean, merged customer behavior";

-- COMMAND ----------

APPLY CHANGES INTO banking.customer_patterns_silver
FROM stream(customer_patterns_silver_copy)
  KEYS (customer_id)
...
-- COMMAND ----------
 ...
 CREATE OR REPLACE STREAMING LIVE TABLE checking_account_silver_copy
 (
   CONSTRAINT customer_id EXPECT (customer_id IS NOT NULL) ON VIOLATION DROP ROW
 )
 TBLPROPERTIES ("quality" = "silver")
 COMMENT "Cleansed bronze customer view (i.e. what will become Silver)"
 AS SELECT json.*
 FROM stream(banking.checking_account_bronze)
 ;

-- COMMAND ----------

 CREATE OR REFRESH STREAMING LIVE TABLE  checking_accounts_silver
 TBLPROPERTIES ("quality" = "silver")
 COMMENT "Clean, merged customer behavior" 
 as select * from stream(checking_account_silver_copy)
